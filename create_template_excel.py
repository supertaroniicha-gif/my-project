...
headers = ['日付', 'Good/More', '項目', '詳細', 'Next Action①', 'Next Action②', 'FB回数']
...
# サンプルデータ
sample_data = [
    ['2024-05-04', 'Good', 'コミュニケーション', '教育プランナーが父にも子供にも話しかけ・目線を合わせた', 'アイスブレイク拡張', '深掘り強化', '1'],
    ['2024-05-04', 'Good', '心理的配慮', '褒めるだけでなく共感を重視', '共感重視', 'ラポール深化', '1'],
    ['2024-05-04', 'More', '勉強内容の深掘り', '毎回勉強について深掘りが不足', 'ヒアリング深化', '質問形式工夫', '1'],
    ['2024-05-05', 'More', '説得フロー', '1課題にすぐトライの良さをぶつけるのはきつい', '段階的説得', '説得フロー改善', '2'],
]
...
ws_feedback.column_dimensions['B'].width = 12
ws_feedback.column_dimensions['C'].width = 20
ws_feedback.column_dimensions['D'].width = 40
ws_feedback.column_dimensions['E'].width = 20
ws_feedback.column_dimensions['F'].width = 20
ws_feedback.column_dimensions['G'].width = 10
...
code_info = [
    ...
    ['7. 権限を許可', ''],
    ['', ''],
    ['📝 コードは別紙ファイル「google_apps_script_v2.gs」を参照', ''],
    ['または GitHub リポジトリから取得してください', ''],
    ['', ''],
    ['✅ セットアップ完了後:', ''],
    ['- フィードバック入力シートにデータを入力', ''],
    ['- 統計ダッシュボードが自動で更新される', ''],
...
for row_idx, row_data in enumerate(code_info, 1):
    for col_idx, value in enumerate(row_data, 1):
        ...
        if 'Google Apps Script' in str(value):
            cell.font = Font(bold=True, size=12)
        elif '1.' in str(value) or '2.' in str(value) or row_idx == 16:
...
# ファイルを保存
output_file = '/home/user/my-project/フィードバック管理システム_テンプレート.xlsx'
wb.save(output_file)

print(f"✅ テンプレートExcelファイルが作成されました！")
print(f"📄 ファイル: {output_file}")
print(f"\n📋 シート構成:")
print(f"   1. 📖 使い方 - セットアップガイド")
print(f"   2. フィードバック入力 - データ入力用（サンプル4行含む）")
